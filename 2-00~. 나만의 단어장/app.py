...
# client = MongoClient('내 AWS아이피', 27017, username="아이디", password="비밀번호") 로도 할 수 있음.
...
@app.route('/detail/<keyword>') # <>안의 내용을 python에서 변수로 받아 쓸 수 있다. /detail?keyword="내용" 일 경우 request.args.get('keyword')를 따로 변수로 받은 다음, 그 변수를 써야 하는 불편함을 줄일 수 있음.
def detail(keyword):
